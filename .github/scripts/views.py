...
<!-- NAV SHELL v5: Auto-hide + search moved here -->
    ...
    <div class="nav-inner">
        ...
        <div class="header-row">
            ...
            <div class="header-actions">
                ...
                <button class="icon-btn" id="settingsBtn" onclick="toggleSettings(event)" title="Cài đặt">⚙️
                    <div class="settings-dropdown" id="settingsDropdown">
                        <a class="settings-item" href="https://t.me/+uhoygGN-1Gc4NzZl" target="_blank" rel="noopener noreferrer">
                            📱 Telegram
                        </a>
                        <button class="settings-item" onclick="showToast('💝 Hỗ trợ: Sẽ cập nhật sớm');">
                            💝 Support/Donate
                        </button>
                        <button class="settings-item" onclick="showToast('📝 Đăng ký: Sẽ cập nhật sớm');">
                            📝 Đăng ký
                        </button>
                        <button class="settings-item" onclick="showToast('🔐 Đăng nhập: Sẽ cập nhật sớm');">
                            🔐 Đăng nhập
                        </button>
                    </div>
                </button>
...
def build_view(output_file, title, json_path, data_key, default_tab="apps", repo_type="apps"):
    ...
    html = html.replace('{repo_name}', config.REPO_NAME)
    html = html.replace('{tint}', config.TINT_COLOR)
    html = html.replace('{json_path}', rel_json_path)
    html = html.replace('{data_key}', data_key)
    html = html.replace('{active_apps}', active_apps)
    html = html.replace('{active_tweaks}', active_tweaks)
    html = html.replace('{active_dylibs}', active_dylibs)
    html = html.replace('{default_icon}', config.SOURCE_LOGO)
    html = html.replace('{repo_type}', repo_type)
...
if __name__ == "__main__":
    ...
    print("🎉 HTML views v5 tạo thành công!")
