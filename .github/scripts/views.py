...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }}
        ...
        /* ─────────────────────────── VERSION POPOVER (neo tại chỗ, dùng cho nút Nhận) ─────────────────────────── */
        ...
        .version-popover-overlay.active {{ pointer-events: auto; }}
        ...
        .version-popover {{
            ...
            transition: opacity 0.18s cubic-bezier(0.4, 0, 0.2, 1),
                        transform 0.18s cubic-bezier(0.4, 0, 0.2, 1);
            transform-origin: var(--popover-origin-x, 100%) var(--popover-origin-y, 0%);
            z-index: 401;
            overflow-y: auto;
            overscroll-behavior: contain;
        }}
...
<!-- VERSION POPOVER (LIST) — popover nhỏ neo tại chỗ ngay cạnh nút "Nhận" vừa nhấn -->
...
<div id="modal" class="modal" onclick="closeModal(event)">
    <div class="modal-content" onclick="event.stopPropagation()">
        ...
        <div class="modal-scroll-body">
            ...
            <div class="info-box" id="changelogBox" style="display:none;">
                <div class="detail-section-title" style="margin-bottom:10px;">🆕 Có gì mới</div>
...
<script>
    ...
    function openModal(idx) {{
        ...
        changelogBox.style.display = hasAnyChangelog ? 'block' : 'none';
    ...
    function openVersionPicker() {{
        ...
        const html = group.versions.map((v, i) => {{
            const label = 'v' + v.version + ' - ' + getCategory(group, v);
            return `
    ...
    // ════════════════════════════════════════════════════════════
    // VERSION POPOVER (LIST) — popover nhỏ neo ngay cạnh nút "Nhận" vừa nhấn
    // ════════════════════════════════════════════════════════════
    ...
    function positionVersionPopover(btnEl) {{
        const popover = document.getElementById('versionPopover');
        const rect = btnEl.getBoundingClientRect();
        const margin = 10;
        const popoverWidth = Math.min(260, window.innerWidth - 32);
        const popoverMaxHeight = 280;

        // Mặc định: bung lên phía trên nút, căn phải trùng mép phải nút
        let left = rect.right - popoverWidth;
        let top = rect.top - popoverMaxHeight - margin;
        let originY = '100%';

        // Nếu không đủ chỗ phía trên, bung xuống phía dưới nút thay vào đó
        if (top < margin) {{
            top = rect.bottom + margin;
            originY = '0%';
        }}
        // Giữ popover trong khung nhìn theo chiều ngang
        left = Math.max(margin, Math.min(left, window.innerWidth - popoverWidth - margin));

        popover.style.left = left + 'px';
        popover.style.top = top + 'px';
        const originX = ((rect.right - left) / popoverWidth * 100).toFixed(0) + '%';
    ...
    window.addEventListener('resize', () => {{
        if (popoverAnchorBtn && document.getElementById('versionPopover').classList.contains('active')) {{
            positionVersionPopover(popoverAnchorBtn);
        }}
    }});
    ...
    function confirmDownloadProceed() {{
        ...
        const filename = version.filename || (group.name + '_v' + version.version);
