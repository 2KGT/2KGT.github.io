...
# ✅ COMPILE REGEX ONCE — Tránh recompile mỗi lần dùng
_STALE_MARKERS_REGEX = re.compile(r'từ Kyic Store|cung cấp bởi')
...
def smart_truncate_description(text, max_chars=None):
    ...
    """
    DEFAULT = "Được tổng hợp by Kyic Store."
    ...
    # ✅ Nhánh 2: Chứa chuỗi rác/tự động cũ — dùng compiled regex
    if _STALE_MARKERS_REGEX.search(clean):
        return DEFAULT
...
def download_resource_to_local(url, target_path):
    ...
    ✅ OPTIMIZED: 
    - Exception handling gom lại 1 chỗ (không lồng try-except)
    - Kiểm tra kích thước file sau khi tải
    - Tránh lưu file rỗng (0 bytes)
    """
    ...
    try:
        ...
        # ✅ Cleanup temp file (gom lại 1 chỗ)
        try:
            os.remove(temp_path)
        except Exception:
            pass
