...
def get_optimized_description(clean_name, version, release_note=None):
    ...
    app_desc_dir = os.path.join(config.REPO_ROOT, 'apps', 'desc', clean_name)
    ...
    if os.path.exists(default_file):
        with open(default_file, 'r', encoding='utf-8') as f: return f.read().strip()
...
def run_feather_engine(release_assets, feather_db):
    ...
    for asset in release_assets:
        if asset["name"].endswith(".ipa"):
            to_process.append({"name": asset["name"], "url": asset["url"], "is_cloud": True, "size": asset["size"], "date": asset["date"], "note": asset.get("body")})
    ...
    for item in to_process:
        ...
        if f_url in feather_db["apps"] and feather_db["apps"][f_url].get("size") == curr_size:
            ...
            apps_map[data['bid']].append({"name": clean_name, "ver": data['ver'], "bid": data['bid'], "dl": f_url, "sz": curr_size, "date": item.get("date", "Local"), "minOS": data['minOS'], "buildVersion": data['buildVersion'], "note": item.get("note")})
        else:
            if item["is_cloud"]:
            ...
            apps_map[bid].append({"name": clean_name, "ver": str(ver), "bid": str(bid), "dl": f_url, "sz": curr_size, "date": item.get("date", "Local"), "minOS": min_os, "buildVersion": build_ver, "note": item.get("note")})
    ...
    for bid, versions in apps_map.items():
        ...
        v_date = latest['date'] if latest['date'] != "Local" else datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        ...
        app_item = {
            ...
            "versions": [{"version": v['ver'], "date": v['date'], "size": v['sz'], "downloadURL": v['dl'], "localizedDescription": get_optimized_description(v['name'], v['ver'], v.get('note'))} for v in versions],
