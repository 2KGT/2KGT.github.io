...
def extract_deb_control_data(path):
    ...
    if not info.get("Package"): 
        info["Package"] = f"com.kyic.{os.path.basename(path).split('_')[0].lower()}"
    if not info.get("Name"): 
        info["Name"] = os.path.basename(path).split('_')[0]
...
def run_sileo_engine(release_assets, system_db):
    print("▶️ Khởi chạy Sileo Engine đám mây kết hợp cục bộ...")
    ...
    for asset in deb_cloud_list:
        ...
        tweaks_map[(bid, arch)].append({"name": tweak_title, "ver": str(ver), "bid": bid, "arch": arch, "dl": f_url, "sz": sz, "desc": deb_info["Description"], "author": deb_info["Author"], "icon": assets["icon"], "safe_file": safe_filename, "section": deb_info["Section"], "is_cloud": True})
        ...
        for root, dirs, files in os.walk(config.DEBS_INPUT_DIR):
            for f_name in files:
                if f_name.endswith(".deb"):
                    ...
                    tweaks_map[(deb_info["Package"], deb_info["Architecture"])].append({"name": deb_info["Name"], "ver": deb_info["Version"], "bid": deb_info["Package"], "arch": deb_info["Architecture"], "dl": relative_path, "sz": int(os.path.getsize(path)), "desc": deb_info["Description"], "author": deb_info["Author"], "icon": assets["icon"], "safe_file": safe_filename, "section": deb_info["Section"], "is_cloud": False})
    ...
    for key in sorted(tweaks_map.keys()):
        for v_item in tweaks_map[key]:
            ...
            # Lưu ý: Cần bổ sung logic tính hash thực tế cho file cloud nếu cần
            md5_hash, sha1_hash, sha256_hash = "0"*32, "0"*40, "0"*64
            if not v_item.get("is_cloud"):
                full_deb_path = os.path.join(config.REPO_OUTPUT_DIR, v_item['dl'])
                if os.path.exists(full_deb_path):
                    with open(full_deb_path, "rb") as f_deb:
