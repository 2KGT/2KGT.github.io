...
def run_dylib_engine(release_assets, dylib_db, system_db):
    """
    ...
    🔑 FIX QUAN TRỌNG: dylib_db là dict cache riêng của dylib (giống
    feather_db/sileo_db ở main.py), được MUTATE TRỰC TIẾP
    (dylib_db["dylibs"][key] = ...) — giống hệt pattern feather_engine.py
    ghi feather_db["apps"][...] và sileo_engine.py ghi system_db["tweaks"][...].
    KHÔNG tự ghi file wikidylibs.json ở đây nữa — main.py sẽ gọi
    data.save_databases() SAU KHI hàm này chạy xong để ghi ra file thật.
    (Bug cũ: hàm này tạo dict cục bộ + tự ghi file ngay, rồi
    save_databases() ở main.py ghi đè lại bằng dylib_db rỗng chưa cập nhật
    → wikidylibs.json luôn thành {"dylibs": {}}.)
    """
    logger.info("🚀 Khởi chạy phân hệ xử lý dữ liệu dylibs chuyên sâu...")

    processed_dylibs_titles = []
    dylib_db.setdefault("dylibs", {})
    wiki_dylibs_data = dylib_db["dylibs"]   # ← THAM CHIẾU trực tiếp (không copy!)
    ...
    for asset in (release_assets if isinstance(release_assets, list) else []):
        ...
        wiki_dylibs_data[f_url] = {
            ...
            "Description": product_description, "Icon": assets.get("icon", ""), 
            "Banner": assets.get("banner", ""),
            "Video": assets.get("video"), "Screenshots": assets.get("screenshots", []),
        ...
        if (str(version), architecture) not in existing_keys:
            dylibs_map[bundle].append({
                ...
                "author": author, "icon": assets.get("icon", ""), "banner": assets.get("banner", ""),
                "video": assets.get("video"), "screenshots": assets.get("screenshots", []),
    ...
    # ── LOCAL .dylib (repo/dylibs/ + subfolder) ────────────────────────────────────
    ...
    # ✅ FIX: Recursively walk subfolder (os.walk thay vì os.listdir)
    dylib_files = []
    for root, dirs, files in os.walk(dylibs_input_dir):
        for file in sorted(files):
            if file.endswith('.dylib') and not file.startswith('.'):
                full_path = os.path.join(root, file)
                dylib_files.append((file, full_path))  # (filename, full_path)
    
    logger.info(f"📂 Quét thư mục local dylib: {dylibs_input_dir}")
    logger.info(f"📦 Tìm thấy {len(dylib_files)} tệp .dylib cục bộ (recursive)")
    
    if not dylib_files:
        logger.warning(f"⚠️ Thư mục {dylibs_input_dir} RỖNG hoặc không có file .dylib")
        logger.info(f"   💡 Để scan local dylibs, thêm .dylib files vào folder này")
        logger.info(f"   💡 Format file: {{bundle}}_{{version}}_{{architecture}}.dylib")
        logger.info(f"      Ví dụ: com.kyic.glow_1.3.1_iphoneos-arm64.dylib")
    else:
        logger.info(f"   📋 File tìm thấy:")
        for fname, fpath in dylib_files:
            rel_path = os.path.relpath(fpath, dylibs_input_dir)
            logger.info(f"      • {rel_path}")

    for f_name, path in dylib_files:
        if f_name.rsplit('.', 1)[0] in processed_safenames:
            continue
        # ✅ path đã có từ os.walk(), không cần tạo lại!
        safe_filename = f_name.rsplit('.', 1)[0]
        id_or_name, dylib_ver, dylib_arch = parse_dylib_filename(f_name)
        
        # ⚠️ Kiểm tra format filename
        if not dylib_ver or not dylib_arch:
            logger.warning(f"⚠️ File {f_name} không đúng format!")
            logger.info(f"   💡 Nên rename thành: {{bundle}}_{{version}}_{{architecture}}.dylib")
            logger.info(f"      Ví dụ: com.kyic.autocat_1.3.1_iphoneos-arm64.dylib")
            logger.info(f"   💡 File sẽ được xử lý với version=1.0, arch=iphoneos-arm64 (mặc định)")
    ...
    # 7. 🔑 FIX: KHÔNG tự ghi wikidylibs.json ở đây nữa!
    # wiki_dylibs_data đã là THAM CHIẾU trực tiếp tới dylib_db["dylibs"]
    # (gán ở đầu hàm), nên mọi entry thêm vào (dòng "wiki_dylibs_data[key] = ...")
    # đã tự động phản ánh vào dylib_db["dylibs"] — main.py sẽ gọi
    # data.save_databases(feather_db, sileo_db, dylib_db) SAU KHI hàm này
    # return để ghi ra file wikidylibs.json thật sự (đồng bộ với cách
    # feather_engine.py/sileo_engine.py hoạt động — KHÔNG tự ghi file
    # trong chính hàm engine).
    # Sắp xếp lại theo Name cho đẹp (không bắt buộc, nhưng giữ nhất quán
    # với thứ tự cũ của wiki_options_db_format).
    sorted_wiki = wiki_options_db_format(wiki_dylibs_data)
    dylib_db["dylibs"] = sorted_wiki
    dylib_db["generated_at"] = gen_time
    logger.info(f"💾 Cache wikidylibs (dylib_db['dylibs']) sẵn sàng: {len(sorted_wiki)} entries")
    logger.info(f"   (sẽ được ghi ra wikidylibs.json khi main.py gọi data.save_databases())")

    # 8. Xuất dylibs.json hoàn chỉnh ra repo root
    ...
    try:
        ...
        logger.info(f"✅ Đã kết xuất dylibs.json hoàn chỉnh: {dylibs_output_path}")
        logger.info(f"   📦 Tổng dylib: {len(final_dylibs)}")
        for app in final_dylibs:
            logger.info(f"      • {app['name']} ({app['bundleIdentifier']}) - {len(app.get('versions', []))} versions")
    except Exception as e:
        logger.error(f"❌ Lỗi ghi tệp dylibs.json: {e}")

    # Summary report
    logger.info("\n" + "=" * 70)
    logger.info("📊 DYLIB ENGINE SUMMARY")
    logger.info("=" * 70)
    logger.info(f"✅ Processed dylibs: {len(processed_dylibs_titles)}")
    logger.info(f"📦 Final dylibs.json apps: {len(final_dylibs)}")
    logger.info(f"💾 wikidylibs.json cache: {len(wiki_dylibs_data)} entries")
    logger.info("=" * 70 + "\n")
