...
def get_release_assets():
    ...
    FIX: Hỗ trợ phân trang (pagination) — GitHub API mặc định chỉ trả 30 release/trang,
    nếu repo có nhiều release thì các trang sau bị bỏ qua hoàn toàn.
    """
    ipa_assets = []
    deb_assets = []
    dylib_assets = []
    ...
    while page <= MAX_PAGES:
        ...
        try:
            ...
            # FIX: Thêm Accept header chuẩn GitHub API
            ...
            for release in releases:
                # Bỏ qua bản draft và pre-release
                if release.get("draft"):
                    continue
                # FIX: Bỏ qua pre-release nếu không muốn phân phối bản thử nghiệm
                # Bỏ comment dòng dưới nếu muốn lọc prerelease
                ...
                # FIX: Lấy Release title (name) + Release notes (body) → gom thành desc đầy đủ
                # Nếu admin viết:
                #   Title: "Telegram v18.0 - Dark Mode Update"
                #   Body: "✨ Thêm Dark Mode\n🐛 Fix crash..."
                # → desc txt sẽ có cả title + body
                release_title = release.get("name") or tag_name or ""
                release_body = release.get("body") or ""
                
                # Gom title + body thành changelog đầy đủ
                full_changelog = ""
                if release_title:
                ...
                for asset in release.get("assets", []):
                    ...
                    # FIX: Bỏ qua asset không có URL hợp lệ
                    ...
                    if asset_name.endswith(".ipa"):
                        ipa_assets.append({
                            ...
                            "size": size,
                            "date": release_date,
                            # FIX: Dùng full changelog (title + body) làm mô tả
                            "body": full_changelog
                        })
        ...
        # FIX: Phân biệt lỗi HTTP (401, 403, 404...) với lỗi mạng
